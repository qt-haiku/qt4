...
static void qt_haiku_draw_button(QPainter *painter, const QRect &rect, bool def, bool flat, bool pushed, bool focus, bool enabled)
{
	...
	QColor panelBgColor = mkQColor(ui_color(B_PANEL_BACKGROUND_COLOR));
	QColor buttonBgColor = mkQColor(tint_color(ui_color(B_PANEL_BACKGROUND_COLOR), B_LIGHTEN_1_TINT));
	QColor lightColor;
	QColor maxLightColor;
	QColor maxShadowColor = mkQColor(tint_color(ui_color(B_PANEL_BACKGROUND_COLOR), B_DARKEN_MAX_TINT));
	QColor dark1BorderColor;
	QColor dark2BorderColor;
	QColor bevelColor1;
	QColor bevelColor2;
	QColor bevelColorRBCorner;
	QColor borderBevelShadow;
	QColor borderBevelLight;
					
	if(enabled) {
		lightColor = mkQColor(tint_color(ui_color(B_PANEL_BACKGROUND_COLOR), B_LIGHTEN_2_TINT));
		maxLightColor = mkQColor(tint_color(ui_color(B_PANEL_BACKGROUND_COLOR), B_LIGHTEN_MAX_TINT));
		dark1BorderColor = mkQColor(tint_color(ui_color(B_PANEL_BACKGROUND_COLOR), B_DARKEN_3_TINT));
		dark2BorderColor = mkQColor(tint_color(ui_color(B_PANEL_BACKGROUND_COLOR), B_DARKEN_4_TINT));
		bevelColor1 = mkQColor(tint_color(ui_color(B_PANEL_BACKGROUND_COLOR), B_DARKEN_2_TINT));
		bevelColor2 = panelBgColor;
		if(def) {
			borderBevelShadow = mkQColor(tint_color(tint_color(ui_color(B_PANEL_BACKGROUND_COLOR), B_DARKEN_3_TINT),
								(B_NO_TINT + B_DARKEN_1_TINT) / 2));
			borderBevelLight = mkQColor(tint_color(tint_color(ui_color(B_PANEL_BACKGROUND_COLOR), B_DARKEN_3_TINT), 
								B_LIGHTEN_1_TINT));
			borderBevelLight.setRed((borderBevelLight.red() + panelBgColor.red()) / 2);
			borderBevelLight.setGreen((borderBevelLight.green() + panelBgColor.green()) / 2);
			borderBevelLight.setBlue((borderBevelLight.blue() + panelBgColor.blue()) / 2);
			dark1BorderColor = mkQColor(tint_color(tint_color(ui_color(B_PANEL_BACKGROUND_COLOR), B_DARKEN_3_TINT), B_DARKEN_3_TINT));
			dark2BorderColor = mkQColor(tint_color(tint_color(ui_color(B_PANEL_BACKGROUND_COLOR), B_DARKEN_3_TINT), B_DARKEN_4_TINT));
			bevelColorRBCorner = borderBevelShadow;			
		} else {
			borderBevelShadow = mkQColor(tint_color(ui_color(B_PANEL_BACKGROUND_COLOR),(B_NO_TINT + B_DARKEN_1_TINT)/2));
			borderBevelLight = buttonBgColor;
			bevelColorRBCorner = dark1BorderColor;
		}
	} else {
		lightColor = mkQColor(tint_color(ui_color(B_PANEL_BACKGROUND_COLOR), B_LIGHTEN_2_TINT));
		maxLightColor = mkQColor(tint_color(ui_color(B_PANEL_BACKGROUND_COLOR), B_LIGHTEN_1_TINT));
		dark1BorderColor = mkQColor(tint_color(ui_color(B_PANEL_BACKGROUND_COLOR), B_DARKEN_1_TINT));
		dark2BorderColor = mkQColor(tint_color(ui_color(B_PANEL_BACKGROUND_COLOR), B_DARKEN_2_TINT));
		...
		if (def) {
			...
			dark1BorderColor = mkQColor(tint_color(tint_color(ui_color(B_PANEL_BACKGROUND_COLOR), B_DARKEN_1_TINT),
								 B_DARKEN_1_TINT));
			dark2BorderColor = mkQColor(tint_color(tint_color(ui_color(B_PANEL_BACKGROUND_COLOR), B_DARKEN_1_TINT),
								 1.16));
		} else {
		...
		bevelColorRBCorner = mkQColor(tint_color(ui_color(B_PANEL_BACKGROUND_COLOR), 1.08));
	}
		
	if(flat) {
		buttonBgColor = panelBgColor;
	}
						
	if (enabled && pushed && !flat) {
		buttonBgColor = mkQColor(tint_color(ui_color(B_PANEL_BACKGROUND_COLOR), B_DARKEN_1_TINT));
		maxLightColor = bevelColor1;
		lightColor = dark1BorderColor;
	}
			
	painter->fillRect(fillArea,buttonBgColor);
	QRect rect1= fillArea;
	
	if(!flat) {
		painter->setPen(QPen(borderBevelShadow));
		painter->drawLine(rect1.bottomLeft(),rect1.topLeft());
		painter->drawLine(QPoint(rect1.left()+1,rect1.top()),rect1.topRight());
		painter->setPen(QPen(borderBevelLight));
		painter->drawLine(QPoint(rect1.right(),rect1.top()+1),rect1.bottomRight());
		painter->drawLine(QPoint(rect1.left()+1,rect1.bottom()),QPoint(rect1.right()-1,rect1.bottom()));
							
		rect1 = rect1.adjusted(1, 1, -1, -1);
		painter->setPen(QPen(dark1BorderColor));
		painter->drawLine(rect1.bottomLeft(),rect1.topLeft());
		painter->drawLine(QPoint(rect1.left()+1,rect1.top()),rect1.topRight());
		painter->setPen(QPen(dark2BorderColor));
		painter->drawLine(QPoint(rect1.right(),rect1.top()+1),rect1.bottomRight());
		painter->drawLine(QPoint(rect1.left()+1,rect1.bottom()),QPoint(rect1.right()-1,rect1.bottom()));

		rect1 = rect1.adjusted(1, 1, -1, -1);
		painter->setPen(QPen(buttonBgColor));
		painter->drawLine(rect1.topLeft(),rect1.topLeft());
		painter->setPen(QPen(lightColor));
		painter->drawLine(QPoint(rect1.left(),rect1.top()+1),QPoint(rect1.left(),rect1.bottom()-1));			
		painter->setPen(QPen(bevelColor2));
		painter->drawLine(rect1.bottomLeft(),rect1.bottomLeft());
		painter->setPen(QPen(lightColor));
		painter->drawLine(QPoint(rect1.left()+1,rect1.top()),QPoint(rect1.right()-1,rect1.top()));			
		painter->setPen(QPen(bevelColor2));
		painter->drawLine(rect1.topRight(),rect1.topRight());
		painter->setPen(QPen(bevelColor1));
		painter->drawLine(QPoint(rect1.left()+1,rect1.bottom()),QPoint(rect1.right()-1,rect1.bottom()));			
		painter->setPen(QPen(bevelColorRBCorner));
		painter->drawLine(rect1.bottomRight(),rect1.bottomRight());
		painter->setPen(QPen(bevelColor1));
		painter->drawLine(QPoint(rect1.right(),rect1.bottom()-1),QPoint(rect1.right(),rect1.top()+1));			

		//rect1 = rect1.adjusted(1, 1, -1, -1);
		painter->setPen(QPen(maxLightColor));
		painter->drawLine(QPoint(rect1.left(),rect1.top()),QPoint(rect1.left(),rect1.bottom()-1));			
		painter->setPen(QPen(buttonBgColor));
		painter->drawLine(rect1.bottomLeft(),rect1.bottomLeft());
		painter->setPen(QPen(maxLightColor));
		painter->drawLine(QPoint(rect1.left()+1,rect1.top()),QPoint(rect1.right()-1,rect1.top()));			
		...
		painter->drawLine(rect1.topRight(),rect1.topRight());
		painter->setPen(QPen(bevelColor2));
		painter->drawLine(QPoint(rect1.left()+1,rect1.bottom()),QPoint(rect1.right(),rect1.bottom()));			
		painter->drawLine(QPoint(rect1.right(),rect1.bottom()-1),QPoint(rect1.right(),rect1.top()+1));
	}	

	if(focus && enabled) {
			painter->setPen(QPen(mkQColor(ui_color(B_NAVIGATION_BASE_COLOR)), 0));
			painter->drawRect(fillArea.adjusted(1,1,-2,-2));
	}

/*	if(def) {
			painter->setPen(QPen(QColor(172,172,172)));
			painter->drawRect(fillArea);
			painter->drawRect(fillArea.adjusted(-1,-1,1,1));
	}			*/
}
